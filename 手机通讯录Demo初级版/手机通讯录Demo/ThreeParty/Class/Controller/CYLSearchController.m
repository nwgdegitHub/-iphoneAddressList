...
- (UIView *)searchBackgroundView
{
    if (_searchBackgroundView == nil) {
        ...
        _searchBackgroundView.backgroundColor = [UIColor purpleColor];
...
- (void)showInViewController:(UIViewController *)controller
{
    ...
    NSLog(@"delegate.navigationController: %@",delegate.navigationController);//(null)
    NSLog(@"self.searchBackgroundView: %@",self.searchBackgroundView);// frame = (0 44; 414 736); alpha = 0;
    [delegate.navigationController.view addSubview:self.navigationController.view];
    
    //仅修改self.navigationController.view的y,xwh值不变 这个设置的目的是看起来顶上去的一样
    self.navigationController.view.frame = CGRectMake(self.navigationController.view.frame.origin.x,
                                                      44,
                                                      self.navigationController.view.frame.size.width,
    ...
    [UIView animateWithDuration:0.25f animations:^{
        //仅修改self.navigationController.view的y,xwh值不变 这一步是用搜索框完全覆盖原导航栏
        self.navigationController.view.frame = CGRectMake(self.navigationController.view.frame.origin.x,
                                                          0,
                                                          self.navigationController.view.frame.size.width,
                                                          self.navigationController.view.frame.size.height
